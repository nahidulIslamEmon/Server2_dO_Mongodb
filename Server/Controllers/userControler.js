...
exports.forgetPassword = catchAsyncError(async (req, res, next) => {
  ...
  const message = `Your Dimvaji Password Reset Link is : ${resetPasswordUrl}`;
...
exports.resetPassword = catchAsyncError(async (req, res, next) => {
  ...
  if (!user)
    return next(
      new ErrorHandler(
        "Invalid Password Reset Token or Token Session Expired",
        400
      )
    );
...
exports.changePassword = catchAsyncError(async (req, res, next) => {
  ...
  if (!isPasswordMatched)
    return next(
      new ErrorHandler("You have entered incorrect old password", 400)
    );
